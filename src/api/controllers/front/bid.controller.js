...
exports.create = async (req, res, next) => {
    try {
        ...
        return res.send({ success: true, message: 'You have placed bid successfully', bid })
    } catch (error) {
        return next(error)
    }
}
...
exports.accept = async (req, res, next) => {
    try {
        ...
        if (bidId) {
            // accept bid
            const bid = await Bid.findByIdAndUpdate({ _id: bidId }, { isAccepted: true }, { new: true })
             console.log('acceptenace for ', bid)
            if (bid) {
                // expire other bids
                console.log('bids for single bid', bid)
                await Bid.updateMany({ _id: { $ne: ObjectId(bidId) }, nftId: bid.nftId }, { $set: { isClaimable: true, isExpired: true } })
                console.log('compiled')
                console.log('bid object', bid.nftId)
                const nft = await Nft.findById({ _id: bid.nftId })
               
                console.log('sold', nft)
                await Nft.updateOne({ _id: bid.nftId },
                    {
                        $unset: {
                ...
                return res.send({ success: true, message: 'Bid has been accepted successfully' })
