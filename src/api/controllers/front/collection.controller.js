...
exports.create = async (req, res, next) => {
    try {
        ...
        const existingCollection = await Collection.findOne({ name: new RegExp(payload.name, 'gi'), userId: payload.userId })

        if (existingCollection)
        {

            return res.status(400).send({ success: false, message: 'Collection with same name already exists' })
        }

        // console.log("reached on collection")
        const collection = await Collection.create(payload)
        // console.log(collection,"collection==========>")
        return res.status(200).send({ success: true, message: 'Collection created successfully', collection })
    } catch (error) {
...
exports.edit = async (req, res, next) => {
    try {
        ...
        if (payload.name) {
            const existingCollection = await Collection.findOne({ _id: { $ne: payload._id }, name: new RegExp(payload.name, 'gi'), userId: payload.userId })
    ...
    try {
        ...
        if (collectionId) {
            ...
            if (collection.deletedCount)
                return res.send({ success: true, message: 'Collection deleted successfully', collectionId })
...
exports.get = async (req, res, next) => {
    try {
        const { collectionId } = req.params
        if (collectionId) {
            let collection = await Collection.findOne({ _id: collectionId }, { __v: 0, createdAt: 0, updatedAt: 0 }).populate([{ path: 'userId', select: '_id username email profilePhoto' }, { path: 'categoryId', select: '_id name image' }]).lean(true)
            if (collection) {
