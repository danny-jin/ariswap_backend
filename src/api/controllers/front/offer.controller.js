            ...
            },
            {
                $project: {
                    offerBy: {
                        ...
                        profilePhoto: { $ifNull: ['$offerBy.profilePhoto', globalImgPlaceholder] }
                    },
...
exports.accept = async (req, res, next) => {
    try {
        ...
        if (offerId) {
            // accept offer
            const offer = await Offer.findByIdAndUpdate({ _id: offerId }, { isAccepted: true }, { new: true })

            if (offer) {
                // expire other offers
                await Offer.updateMany({ _id: { $ne: ObjectId(offerId) }, nftId: offer.nftId }, { $set: { isExpired: true } })

                await NFT.updateOne({ _id: offer.nftId },
                    {
                        $unset: {
                ...
                return res.send({ success: true, message: 'Offer has been accepted successfully' })
            } else return res.status(400).send({ success: false, message: 'Offer not found for given Id' })
        } else
            return res.status(400).send({ success: false, message: 'Offer Id is required' })
    } catch (error) {
